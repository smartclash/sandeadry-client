        ...
        {items.map((item) => (
          <li key={item.label}>
            <a
              ...
              className={cx({ "is-active m-2": item.isRefined })}
              onClick={(e) => {
                e.preventDefault();
                refine(item.value);
              }}
            >
              <div className="tag is-link is-light">{item.count}</div>{" "}
              {item.label}
            </a>
          </li>
        ))}
...
const Sidenav = () => {
  return (
    <div className="menu">
      ...
      <p className="menu-label has-text-weight-bold">Degrees</p>
      <CustomRefinement attribute="Degrees" />
      <hr className="has-text-link" />
      <p className="menu-label has-text-weight-bold">Subjects</p>
      <CustomRefinement attribute="Subject" showMore={true} />
      <hr />
      <p className="menu-label has-text-weight-bold">Topics</p>
